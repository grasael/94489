...
def clean_rural_data(projects_file, outcomes_file, essays_file, output_file):
    """
    Cleans and processes the input datasets, merging them into a single cleaned file.
    Adds an `is_rural` column based on `school_metro`.
    ...
    # Add is_rural feature
    projects["is_rural"] = projects["school_metro"].apply(lambda x: 1 if x == "rural" else 0)
    projects = projects.drop(columns=["school_metro"])
    ...
    print(f"Processed data with rural column saved to {output_file}")
